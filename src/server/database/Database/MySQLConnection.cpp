...
PreparedResultSet* MySQLConnection::Query(PreparedStatementBase* stmt)
{
    ...
    return new PreparedResultSet(mysqlStmt->GetSTMT(), result, rowCount, fieldCount, stmt->m_name);
}
...
PreparedResultSet* MySQLConnection::QueryCustomStatement(uint32 id, PreparedStatementBase* values)
{
    ...
    return new PreparedResultSet(stmnt->GetSTMT(), result, rowCount, fieldCount, values->m_name);
}
// @tswow-end
